...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (!quote.fileUrl || !quote.isUpload) {
    return NextResponse.json({ error: "Kein hochgeladenes PDF vorhanden." }, { status: 400 });
  }
  ...
  const filePath = path.join(process.cwd(), "public", quote.fileUrl);
  ...
  await logActivity(
    workspaceId,
    "QUOTE_SENT",
    `Angebot (Upload) gesendet: ${quote.quoteNumber} an ${quote.customerEmail}`,
    { entityId: quote.id, entityType: "Quote" }
  );
